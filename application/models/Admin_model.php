...
//        echo $this->db->last_query();
    ...
    public function get_tutor_questions($tutor_id){
        ...
        $this->db->where('idea_info.parent_question_id', 0);
        ...
        if(empty($result)){
            ...
            $this->db->join('question_ideas', 'question_ideas.tutor_question_id = tbl_question.id');
            $this->db->join('idea_info', 'idea_info.question_id = tbl_question.id');
            // $this->db->group_by('question_ideas.question_id');
            $query = $this->db->get();
    ...
    public function student_idea_details($question_id,$idea_id,$student_id,$module_id){
        $this->db->select('*, question_ideas.id as idea_id, question_ideas.user_id as user_id');
        $this->db->from('question_ideas');
    ...
    public function get_teacher_idea_correction($idea_id,$student_id,$module_id){
        $this->db->select('*');
    ...
    public function getCourseByUserType($user_type, $country_id,$status)
    {
        if($user_type==6){
            $type = 1;
        }elseif($user_type==2){
