...
//        echo $this->db->last_query();
	...
	public function registeredCourse($user_id)
    {
        ...
        $this->db->where('tbl_course.course_access',1);
        $this->db->group_by('tbl_course.id');
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    public function get_profile_info_creative($user_id)
    {
        $this->db->from('tbl_useraccount');
...
public function getCreativeCourses($user_id, $country_id){
    // SELECT * FROM tbl_course WHERE `courseName` LIKE '%creative%'
    $this->db->select('*');
    $this->db->from('tbl_course');
    $this->db->where('country_id',$country_id);
    $this->db->like('courseName', 'creative');
    $results = $this->db->get()->result_array();
    $all_course = array();
    foreach($results as $course){
        $all_course[] = $course['id'];
    }
    return $all_course;
}
