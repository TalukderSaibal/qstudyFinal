    ...
    <div class="col-md-12 ">
        <div class="panel-group " id="task_accordion" role="tablist" aria-multiselectable="true"> 
            <div class="top_headed">
              ...
              <div>
                <a class="btn btn-info" style="margin-top: 20px;" type="button" id="approve_idea">Admin Approve</a>
              </div>
                    ...
                    <tbody>
                      ...
                      <tr> 
                        <td style="border:0;width: 35px;text-align: center;">
                            <input type="checkbox" class="approval_ideas" data-status="<?=$student_idea['admin_answered']?>" value="<?=$student_idea['idea_id']?>" <?php if($student_idea['examine_status']==1){echo "checked";}?>> 
                        </td>
                        <td style="<?php if($student_idea['examine_status']==1){echo "background-color: #efe4b0;";} if($student_idea['student_ans_check_by']==2){echo "text-decoration:underline;";}?> ">
                          <a href="<?php echo base_url('admin/idea_create_student_setting/'.$student_idea['idea_id'].'/'.$student_idea['idea_no'].'/'.$student_idea['question_id'].'/'.$student_idea['user_id'])?>">  <?php $get_date= date_create($student_idea['submit_date']);echo date_format($get_date,"d/m/yy");?>  </a>
                        </td> 
...
<script>
  $(document).ready(function(){
    $('#approve_idea').click(function(){
       $('.approval_ideas').each(function(){
        
           var idea_status = $(this).attr('data-status');
           if(idea_status==1){
            var idea_id = $(this).val();
              // alert(idea_id);
           }
           
       });
       window.location.href = '<?=base_url()?>/Admin/new_idea_create_tutor';
    });
  });
</script>
