                ...
                <div class="ss_s_b_main" style="min-height: 100vh">
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="storyWriteBody">
                            ...
                            <?php $i = 0;
                            foreach ($question['paragraph'] as $key_2 => $val) {
                                $paragraph_count = isset($val['Paragraph']) ? count($val['Paragraph']) : 0;
                                $PuzzleParagraph = isset($val['PuzzleParagraph']) ? count($val['PuzzleParagraph']) : 0;
                                $RightAnswer = isset($val['RightAnswer']) ? count($val['RightAnswer']) : 0;
                                $totalCount  = $RightAnswer + $paragraph_count + $PuzzleParagraph;
                                $sl_no = $key_2;  ?>
                                <div class="storyWriteParts">
                                    <?php foreach ($val as $key_3 => $value) {  ?>
                                        <?php foreach ($value as $key_4 => $values) {
                                            $i++;  ?>
                                            <div class="firstElementStoryWrite paragraphFirstEm<?= $key_2; ?>"> <?= $values; ?> </div>

                                            <?php if ($key_3 == "Paragraph") {
                                                $val[1] = "right_ones_xxx"; ?>
                                            ...
                                            <?php if ($key_3 == "RightAnswer") {
                                                $val[1] = "rightOne"; ?>
                                                <div class="chooseBtnStoryWrite"> <button type="button" id="paragraphBtn" class="paragraphBtn_<?= $key_4; ?> paragraphBtn 4 btn offparagraph_<?= $key_2; ?> disabledButton<?= $i; ?>" style="display: none;" onclick="rightOrWrongParagraph('<?= $val[1]; ?>' , '<?= $values; ?>', 'paragraphBtn_<?= $key_4; ?>' , 'paragraphBtn' , 4 , '<?= $key_2; ?>' , '<?= $totalCount; ?>' , '<?= $i; ?>')">Choose</button></div>
                                            <?php } ?>

                                            <?php if ($key_3 == "PuzzleParagraph") {
                                                $val[1] = "wrong_ones_xxx"; ?>
                                                <div class="chooseBtnStoryWrite"> <button type="button" id="paragraphBtn" class="paragraphBtn_<?= $key_4; ?> paragraphBtn 4 btn offparagraph_<?= $key_2; ?> disabledButton<?= $i; ?>" style="display: none;" onclick="rightOrWrongParagraph('<?= $val[1]; ?>' , '<?= $values; ?>', 'paragraphBtn_<?= $key_4; ?>' , 'paragraphBtn' , 4 , '<?= $key_2; ?>' , '<?= $totalCount; ?>' , '<?= $i; ?>')">Choose</button></div>
                                            <?php } ?>

                                            <?php if ($key_3 == "WrongAnswer") {
                                                $val[1] = $question['wrongParagraphIncrement'][$key_2]['WrongAnswer'][$key_4]; ?>
                                                <div class="chooseBtnStoryWrite"> <button type="button" id="paragraphBtn" class="paragraphBtn_<?= $key_4; ?> paragraphBtn 4 btn offparagraph_<?= $key_2; ?> disabledButton<?= $i; ?>" style="display: none;" onclick="rightOrWrongParagraph('<?= $val[1]; ?>' , '<?= $values; ?>', 'paragraphBtn_<?= $key_4; ?>' , 'paragraphBtn' , 4 , '<?= $key_2; ?>' , '<?= $totalCount; ?>' , '<?= $i; ?>')">Choose</button></div>
                                            <?php } ?>
